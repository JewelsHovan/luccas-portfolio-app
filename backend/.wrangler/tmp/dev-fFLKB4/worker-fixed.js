...
// .wrangler/tmp/bundle-b7DmmB/strip-cf-connecting-ip-header.js
...
var imageCache = {
  ...
  overlayImages: [],
  lastFetch: null,
  cacheTimeout: 4 * 60 * 60 * 1e3
  // 4 hours (temp links expire)
};
...
__name(dropboxApiCall, "dropboxApiCall");
async function getAllImagesFromFolder(accessToken, folderPath) {
  ...
  async function scanFolder(path) {
    try {
        ...
        if (entry[".tag"] === "file") {
          ...
          if (name.endsWith(".jpg") || name.endsWith(".jpeg") || name.endsWith(".png") || name.endsWith(".gif") || name.endsWith(".webp")) {
            try {
              ...
              console.error(`Error getting link for ${entry.name}:`, error3.message);
            }
          }
      ...
      console.error(`Error scanning folder ${path}:`, error3.message);
      throw error3;
    }
  }
...
__name(getAllImagesFromFolder, "getAllImagesFromFolder");
async function refreshImageCache(accessToken) {
  ...
  try {
    ...
    console.error("Error refreshing cache:", error3);
    throw error3;
  }
}
__name(refreshImageCache, "refreshImageCache");
...
var worker_fixed_default = {
  async fetch(request, env2, ctx) {
    const url = new URL(request.url);
    if (request.method === "OPTIONS") {
      return handleCors(request, new Response(null, { status: 204 }));
    }
    if (!env2.DROPBOX_ACCESS_TOKEN) {
    ...
    if (url.pathname === "/api/images" && request.method === "GET") {
      try {
        const needsRefresh = !imageCache.lastFetch || Date.now() - imageCache.lastFetch > imageCache.cacheTimeout || imageCache.baseImages.length === 0 || imageCache.overlayImages.length === 0;
        if (needsRefresh) {
          await refreshImageCache(env2.DROPBOX_ACCESS_TOKEN);
        }
        ...
        console.error("Error fetching images:", error3);
        return handleCors(request, new Response(JSON.stringify({
    ...
    if (url.pathname === "/api/debug" && request.method === "GET") {
      try {
        const testResponse = await dropboxApiCall("users/get_current_account", env2.DROPBOX_ACCESS_TOKEN);
        return handleCors(request, new Response(JSON.stringify({
...
// .wrangler/tmp/bundle-b7DmmB/middleware-insertion-facade.js
var __INTERNAL_WRANGLER_MIDDLEWARE__ = [
  middleware_ensure_req_body_drained_default,
  middleware_miniflare3_json_error_default
];
var middleware_insertion_facade_default = worker_fixed_default;
...
// .wrangler/tmp/bundle-b7DmmB/middleware-loader.entry.ts
...
//# sourceMappingURL=worker-fixed.js.map
